...
class ResolutionSliderHandler {
  ...
  init(display: Display) {
    ...
    icon.image = NSImage(systemSymbolName: "arrow.up.backward.and.arrow.down.forward", accessibilityDescription: self.title)
    icon.contentTintColor = NSColor.black.withAlphaComponent(0.6)
    icon.frame = NSRect(x: view.frame.origin.x + 6, y: view.frame.origin.y + 16, width: 15, height: 15)
